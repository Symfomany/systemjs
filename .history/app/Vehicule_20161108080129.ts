...
import * as request from 'request';
    ...
    export class Vehicule implements MotorInterface {
        ...
        public getProprietaire(nat:string = "fr"){
            let info:any = null;
             request.get(`https://randomuser.me/api/?nat=${nat}`, function (error, response, body) {
         ...
         * @memberOf Vehicule
         */
        public getInteresses(limit:number = 1000){
            let info:any = null;
             request.get(`https://randomuser.me/api/?results=${limit}`, function (error, response, body) {
                if (!error && response.statusCode === 200) {
                     info = JSON.parse(body);
                }
            });
